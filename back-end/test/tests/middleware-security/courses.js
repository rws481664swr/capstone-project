...
describe('courses middleware security', () => {
    ...
    it("should not allow users to get unless a member of course", async () => {
            const c3 = await createCourse(newCourse())
            try {
                await requests.get.student(prefix(c3._id))
                should.fail('fail')
            } catch (e) {
                e.message.should.not.equal('fail')
                e.response.status.should.eql(401)
                e.response.data.message.should.eql('user is not a member of the course')
            }
        }   )


            it("should not allow users to get unless a member of course unless admin", async () => {
                const c3 = await createCourse(newCourse())
                await requests.get.admin(prefix(c3._id))
            })
