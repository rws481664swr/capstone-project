...
describe('user api routes tests', function () {
    ...
    describe("GET", () => {
        ...
        it(`should get all users`, async () => {
            ...
            const expected = jsonify(await Users.find({}).sort({username: 1}).exec())
            data.should.eql(expected)
        })
        it(`should get all users in reverse order`, async () => {


            const {data} = await axios.get(prefix, {...adminTokenConfig, params: {username: 'desc'}})
        ...
        it(`should get user with username u1`, async () => {
            ...
            const expected = await Users.findOne({username: 'u1'}).populate('courses').exec()
